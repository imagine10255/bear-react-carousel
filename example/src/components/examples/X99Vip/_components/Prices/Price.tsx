...
import {Flex, FCProps} from '@acrool/react-grid';
...
const Price = ({
    ...
    const data = [
        'Game venues: no restriction on venue type',
        'Number of times limit: one time',
    ];
    ...
    return <PriceRoot className={className}>
        ...
        <Header>
            ...
            <GiftImage src={asset('/images/x99_vip/gift.png')}/>
            <Amount>
                <strong>
                    ${formatCurrency(amount)}
                </strong>
                <CountText>/time</CountText>
            </Amount>
        </Header>
